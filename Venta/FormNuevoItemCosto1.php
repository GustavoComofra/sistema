...
<div class="form-group"  >
  <table class="table" width="423"  border="0">
    ...
    <tr>
	<td width="156" collapse="2">Num</td>	
    ...
    <tr>
		<td>
		<select name="listFk_Num_Recl_Cost" size="1" id="listFk_Num_Recl_Cost">
     <option value="<?php print $rowNuevoCosto['Fk_Num_Recl_Cost'];?>"><?php print $rowNuevoCosto['Fk_Num_Recl_Cost'];?></option>
        <?php
include("../Conexion/conexion.php");
  
$queryCosto = $mysqli -> query ("SELECT * FROM `ComVerReclamo` ORDER BY `ComVerReclamo`.`NumReclamo` ASC");

 while ($valoresCosto = mysqli_fetch_array($queryCosto))
		  
		  {

 echo '<option value="'.$valoresCosto['NumReclamo'].'">'. $valoresCosto['NumReclamo'] . " - " . $valoresCosto['Reclamo'] .'</option>';
...
$Fk_Num_Recl_Cost=$_POST['listFk_Num_Recl_Cost'];		
...
if(!$Fk_Num_Recl_Cost==null){
	...
	echo "<h1>"."<a href=\"/sistema/Venta/ListCostoReclamo.php\">Volver</a>"."</h1>";
	echo "
